...
class SPADE(object):
    ...
    def build_model(self):
        ...
        prior_det_supercode_mean, prior_det_supercode_logvar = self.prior_code(channel_multiplier=4)#self.encoder_code(self.real_x, scope='prior_det_supercode')
        ...
        prior_nondet_supercode_mean, prior_nondet_supercode_logvar = self.prior_code(channel_multiplier=4)#self.encoder_code(self.real_x, scope='prior_nondet_supercode')
        ...
        [code_det_prior_real_logit, code_det_prior_real_summary], [code_det_prior_fake_logit, code_det_prior_fake_summary] = self.discriminate_code(real_code_img=tf.concat([tf.stop_gradient(fake_det_x_full_ctxcode), random_simple_det_code], -1), fake_code_img=tf.concat([tf.stop_gradient(fake_det_x_full_ctxcode), fake_det_x_code], -1), name='det_prior')
        [code_nondet_prior_real_logit, code_nondet_prior_real_summary], [code_nondet_prior_fake_logit, code_nondet_prior_fake_summary] = self.discriminate_code(real_code_img=tf.concat([tf.stop_gradient(fake_nondet_x_full_ctxcode), random_simple_nondet_code], -1), fake_code_img=tf.concat([tf.stop_gradient(fake_nondet_x_full_ctxcode), fake_nondet_x_code], -1), name='nondet_prior')

        [code_det_gen_real_logit, code_det_gen_real_summary], [code_det_gen_fake_logit, code_det_gen_fake_summary] = self.discriminate_code(real_code_img=tf.concat([tf.stop_gradient(fake_det_x_full_ctxcode), tf.stop_gradient(fake_det_x_code)], -1), fake_code_img=tf.concat([tf.stop_gradient(fake_det_x_full_ctxcode), random_det_code], -1), name='det_gen')
        [code_nondet_gen_real_logit, code_nondet_gen_real_summary], [code_nondet_gen_fake_logit, code_nondet_gen_fake_summary] = self.discriminate_code(real_code_img=tf.concat([tf.stop_gradient(fake_nondet_x_full_ctxcode), tf.stop_gradient(fake_nondet_x_code)], -1), fake_code_img=tf.concat([tf.stop_gradient(fake_nondet_x_full_ctxcode), random_nondet_code], -1), name='nondet_gen')
        ...
        g_det_vgg_loss = self.vgg_weight * VGGLoss()(self.real_x, fake_det_x_stats[0][0])
        ...
        e_nondet_adv_loss = generator_loss(self.code_gan_type, code_nondet_prior_fake_logit)
        e_nondet_kl_loss = kl_loss(x_nondet_code_mean, x_nondet_code_logvar)
        e_nondet_gen_loss = gaussian_loss(fake_nondet_x_code, prior_nondet_code_mean, prior_nondet_code_logvar)
        e_nondet_negent_loss = negent_loss(x_nondet_code_mean, x_nondet_code_logvar)
        e_nondet_gen2_loss = -tf.reduce_mean(prior_dist_nondet_code_dist.log_prob(tf.stop_gradient(fake_nondet_x_code))) / int(fake_nondet_x_code.get_shape()[-1])
        e_nondet_kl2_loss = (e_nondet_gen2_loss + e_nondet_negent_loss)
        ...
        e_nondet_genctx_loss = gaussian_loss(fake_nondet_x_ctxcode, prior_nondet_ctxcode_mean, prior_nondet_ctxcode_logvar)
        ...
        e_nondet_klctx2_loss = (e_nondet_genctx_loss + e_nondet_negentctx_loss)
        ...
        e_det_adv_loss = generator_loss(self.code_gan_type, code_det_prior_fake_logit)
        e_det_kl_loss = kl_loss(x_det_code_mean, x_det_code_logvar)
        e_det_gen_loss = gaussian_loss(fake_det_x_code, prior_det_code_mean, prior_det_code_logvar)
        e_det_negent_loss = negent_loss(x_det_code_mean, x_det_code_logvar)
        e_det_gen2_loss = -tf.reduce_mean(prior_dist_det_code_dist.log_prob(tf.stop_gradient(fake_det_x_code))) / int(fake_det_x_code.get_shape()[-1])
        e_det_kl2_loss = (e_det_gen2_loss + e_det_negent_loss)
        e_det_reg_loss = regularization_loss('encoder_det_code')
        
        e_det_klctx_loss = kl_loss(x_det_ctxcode_mean, x_det_ctxcode_logvar)
        e_det_genctx_loss = gaussian_loss(fake_det_x_ctxcode, prior_det_ctxcode_mean, prior_det_ctxcode_logvar)
        ...
        e_det_klctx2_loss = (e_det_genctx_loss + e_det_negentctx_loss)
        ...
        with tf.control_dependencies([ema.apply([e_det_kl_loss_truncated, e_nondet_kl_loss_truncated])]):
            ...
            self.g_loss = g_nondet_adv_loss + g_nondet_reg_loss + 10*g_nondet_feature_loss + 0*g_nondet_vgg_loss + 10*g_nondet_ce_loss + tf.zeros_like(e_nondet_adv_loss) + e_nondet_reg_loss + 0.05*(0*e_nondet_gen_loss + e_nondet_gen2_loss + (g_nondet_code_ce_loss + 0.1*(0*e_nondet_code_prior_loss + e_nondet_code_prior2_loss + e_nondet_code_negent_loss)) + e_nondet_negent_loss) + 0.0001*e_nondet_klctx2_loss
            self.e_loss = 10*g_det_ce_loss + 10*g_det_segmapce_loss + g_det_reg_loss + 0*e_det_adv_loss + e_det_reg_loss + 0.05*(0*e_det_gen_loss + e_det_gen2_loss + (g_det_code_ce_loss + 0.1*(0*e_det_code_prior_loss + e_det_code_prior2_loss + e_det_code_negent_loss)) + e_det_negent_loss) + 0.0001*e_det_klctx2_loss
        ...
        DE_vars = [var for var in t_vars if 'discriminator_det_prior_code' in var.name or 'discriminator_nondet_prior_code' in var.name or 'discriminator_det_gen_code' in var.name or 'discriminator_nondet_gen_code' in var.name]
        ...
        self.summary_e_det_genctx_loss = tf.summary.scalar("e_det_genctx_loss", e_det_genctx_loss)
        ...
        self.summary_e_det_gen_loss = tf.summary.scalar("e_det_gen_loss", e_det_gen_loss)
        self.summary_e_det_gen2_loss = tf.summary.scalar("e_det_gen2_loss", e_det_gen2_loss)
        ...
        self.summary_e_nondet_genctx_loss = tf.summary.scalar("e_nondet_genctx_loss", e_nondet_genctx_loss)
        ...
        self.summary_e_nondet_gen_loss = tf.summary.scalar("e_nondet_gen_loss", e_nondet_gen_loss)
        self.summary_e_nondet_gen2_loss = tf.summary.scalar("e_nondet_gen2_loss", e_nondet_gen2_loss)
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if (counter - 1) % self.n_critic == 0:
                    ...
                    if (counter - 1) % (self.n_critic*self.code_n_critic) == 0:
                        ...
                        # Update G
                        real_ctx_images, real_x_images, real_x_segmap, fake_det_x, fake_det_x_var, fake_det_x_segmap, fake_nondet_x, random_dist_fake_det_x, random_dist_fake_det_x_segmap, random_dist_fake_nondet_x, random_fake_det_x, random_fake_det_x_segmap, random_fake_nondet_x, nondet_code_vae, _, g_loss, summary_str = self.sess.run(
                ...
                if np.mod(idx + 1, self.print_freq) == 0:
                    ...
                    save_segmaps(random_dist_fake_det_x_segmap, self.color_value_dict, [self.batch_size, 1],
